...
% F = max(выручка/max_revenue, пряжа/max_yarn) -> min
% Преобразуем к виду для linprog: c'x -> min
f = [
    (b1/max_yarn - d1/max_revenue);   % x11
    (b2/max_yarn - d2/max_revenue);   % x12
    (b3/max_yarn - d3/max_revenue);   % x13
    (b1/max_yarn - d1/max_revenue);   % x21
    (b2/max_yarn - d2/max_revenue);   % x22
    (b3/max_yarn - d3/max_revenue)    % x23
];
...
% Вывод результатов
disp('=== ОПТИМАЛЬНЫЙ ПЛАН ПРОИЗВОДСТВА (МЕТОД МИНИМАКС/МАКСИМИН) ===');
