...
# чтение обучающих данных из csv-файла
def read_data():
    df = pd.read_csv('mnist_test.csv',header=None)
    #отбор примеров по признаку соответствия метки класса – для варианта цифр (0,2,4)
    data = df.loc[(df[0] == 0)|(df[0] == 2)|(df[0] == 4)]
...
X_train, X_test, y_train, y_test = train_test_split(X, y, train_size=200, test_size=100, stratify=y, random_state=42)
...
#преобразование выходных значений
#преобразование входных данных в двумерный массив
X_train = X_train.values.reshape(X_train.shape[0], img_rows, img_cols, 1)
X_test = X_test.values.reshape(X_test.shape[0], img_rows, img_cols, 1)
#количество распознаваемых классов - 10
num_classes = 10
#Кодирование целевых меток со значением от 0 до 2.
le = preprocessing.LabelEncoder()
#сопоставление закодированных цифр(y = {0, 2, 4}) числовым меткам (0, 1, 2)
le.fit(y)
#преобразование правильных ответов в one-hot представление: векторы размерностью 3, в которых единственная единица на месте нужной цифры
...
def show_model_lose():
    #построение и вывод графика функции потерь
...
def show_model_accuracy():
    #построение и вывод графика функции точности распознавания
